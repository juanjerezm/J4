...
The calculation procedure to reach uniform units [€/MWh] is done in ['data/_master-data/fuel data.xlsx'](../data/_master-data/fuel%20data.xlsx).


## Electricity tariffs

Transmission network tariffs are included and obtained from `placeholder - wait for Julian's answer`. Consumer-paid tariffs are included, while generator-paid tariffs are disregarded because of their relatively small impact and to keep the model simple.

Distribution network tariffs, which can be obtained from `placeholder - wait for Julian's answer`, are disregarded due to uncertainty about the specific voltage level of WH sources or DH generators.

More details can be found in ["dh-size-and-electricity-tariffs.docx"](../data/_master-data/dh-size-and-electricity-tariffs.docx).
