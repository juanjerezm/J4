...
def build_figure(data: pd.DataFrame, plot_spec: PlotSpec, mappings: Mappings) -> Figure:
    ...
    for (r_idx, c_idx), ax in np.ndenumerate(axes):
        ...
        draw_subplot_line(
            panel_data, ax, panel_label, markers, colors, plot_spec, on_edge
        )
...
def draw_subplot_line(
    ...
    Renders series as lines, grouped by index values.
...
def main() -> None:
    ...
    plot_spec = load_plot_spec(analysis_scope.config / "plot-SADR-NPV.yml")
