...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Generate plot of main operational results by country, policy, and fuel."
    )
...
def summarize_results(
    ...
    """
    Summarize scenario results into a standardized DataFrame aggregated by fuel category,
    with columns ['country', 'policy', 'F', 'level'].
    """
    ...
    # --- summarise results ---
    df = utils.aggregate(df, ["country", "policy", "case", "F"], ["level"])
...
def main(*, specification_path: str) -> None:
    """
    Load scenarios, summarize results, and produce stacked-bar plots by country and policy,
    categorised by fuel source.
    """
    ...
    for (r_idx, c_idx), ax in np.ndenumerate(axes):
        ...
        df = df.pivot_table(index="policy", columns="F", values="level", observed=True)
        df = df.loc[:, df.sum() != 0]  # exclude empty categories
        df.plot(kind="bar", stacked=True, ax=ax, legend=False, color=color)
...
QUICKRUN_ARGS = {}
# QUICKRUN_ARGS = {"specification_path": "configs/plots/Main_HeatProductionChange.yml"}
# QUICKRUN_ARGS = {"specification_path": "configs/plots/Main_CarbonEmissionsChange.yml"}
# QUICKRUN_ARGS = {"specification_path": "configs/plots/Main_FuelConsumptionChange.yml"}
# QUICKRUN_ARGS = {"specification_path": "configs/plots/Main_ElectrProductionChange.yml"}
