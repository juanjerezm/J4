...
def plot_histograms(data, bin_size=10, min_price=0, max_price=500):
    ...
    for ax, year in zip(axes, years):
        ...
        ax.hist(
            year_data, bins=bin_edges, color="blue", alpha=0.7, edgecolor="none", label=f"Year {year}", density=True,
        )

        ax.set_title(f"{year}", fontsize=8, fontweight="bold")
        # ax.set_ylabel("Frequency", fontsize=9)
        ax.grid(axis="y", linestyle="--", alpha=0.4)

    fig.supylabel("Frequency", fontsize=8, fontweight="bold")
