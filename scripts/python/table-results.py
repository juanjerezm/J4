...
class Scenario:
    ...
    def process_data(self, mode) -> None:
        ...
        if mode == 'reference':
            ...
            df = utils.aggregate(df, [aggregator], ["level"])
        if mode == 'integrated':
            df = utils.filter(df, include={"case": "integrated"})
            df = utils.aggregate(df, [aggregator], ["level"])            
        if mode == 'difference':
            df = utils.aggregate(df, ["case", aggregator], ["level"])
        ...
        # Clean up
        df = df[["country", "policy", aggregator, "level"]]
        self.data = df
        return
...
def summary_csv(df: pd.DataFrame, save: bool = False, outdir: Path = Path.cwd(), filename: str = ''):
    ...
    if mode == 'reference':
        ...
        df = df.pivot(index=aggregator, columns="country", values="level")
    if mode == 'integrated':
        df = df.pivot(index=["country", aggregator], columns="policy", values="level")
    if mode == 'difference':
        df = df.pivot(index=["country", aggregator], columns="policy", values="level")
...
def main():
    ...
    df = exclude_empty_category(df, aggregator, tolerance = 1e-3)
    df['level'] = df['level'].round(3)
...
if __name__ == "__main__":
    ...
    # var = "x_h"
    # SCALE = 1e-3
    # table_name = f'table-heat-{mode}'

    ## Electricity
    # var = "x_e"
    # SCALE = 1e-3
    # table_name = f'table-electricity-{mode}'

    ## Emissions
    # var = "w"
    # SCALE = 1e-6
    # table_name = f'table-emissions-{mode}'

    ## Fuel use
    # var = "x_f"
    # SCALE = 1e-3
    # table_name = f'table-fuel-{mode}'

    ## Cooling production
    var = "x_c"
    SCALE = 1e0
    table_name = f'table-cooling-{mode}'
